...
- **索引与范围** — include.txt 编辑保存（语法校验）、添加外部 @ 文件、新建笔记（自动 frontmatter）、
  触发增量索引（实时日志）
- **模型管理**（侧栏「模型」分组，两页）— 生成供应商（10 家预设 + 自定义档案增删改 +
  每档案独立 key + 连通测试）；检索 Embedding（模式四选、HTTP 端点档案带 key、
  内置 llama.cpp 状态、LM Studio 式 GGUF 下载器：浏览文件→行内进度→已下载选用）
- **设置** — 生成偏好（temperature/top_k）、全局 key、当前配置一览
